...
try:
    ...
    from core.models import ShareData
except ImportError:
...
class EnhancedStratumClient:
    ...
    def __init__(self, host, port, user, password, database_manager: Optional[DatabaseManager] = None):
        self.host = host
        self.port = port
        self.user = user
        ...
        self.monitor = StratumMonitor(worker_name=user, database_manager=database_manager)
    ...
    def submit_share(self, extranonce2, ntime, nonce, hash_result):
        ...
        if response and response.get("id") == msg_id and response.get("result") is True:
            ...
            self.monitor.record_share_accepted(job_id=self.job_id)
            return True
        else:
            error_msg = response.get('error') if response else "No response"
            print(f"Share rejected: {error_msg}")
            self.monitor.record_share_rejected(reason=str(error_msg), job_id=self.job_id)
            return False
