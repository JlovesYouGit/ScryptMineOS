...
class AsicTelemetryProessional:
    ...
    # Watts from on-board INA sensor (true wall power)
    power_real: float
    # GH/s total hashrate across all chains  # Add division by zero protection
...
class ProfessionalFleetOptimizer:
    ...
    def __init__(self, fleet_ips: List[str], api_port: int = 4028):
        ...
        # MAX_RETRIES% above median triggers redirect
        self.efficiency_threshold_multiplier = 1.MAX_RETRIES
    ...
    f'http://{ip}:{self.api_port}/api/stats',
     timeout = self.query_timeout)
            ...
            if isinstance(chain_rates, list) and len(chain_rates) > 0:
                ...
                total_hash_rate = sum(float(rate)
                                      for rate in chain_rates) / 1e9
            ...
            joules_per_th = (power_real / (total_hash_rate * 1000)
                             ) if total_hash_rate > 0 else 999.0
            ...
            accept_rate = (accepted / (accepted + rejected)) *
                           100 if (accepted + rejected) > 0 else 100
    ...
    def calculate_median_jth(
        ...
        logger.info(
            f"Fleet median J/TH: {median_jth:.3f} ({len(online_units)} units)")
        return median_jth
    ...
    def identify_underperformers_professional(
        ...
        for unit in fleet:
            ...
            fan_fail = any(
    rpm == 0 for rpm in unit.fan_rpm) if unit.fan_rpm else False
            ...
            if efficiency_fail
                ...
                if efficiency_fail:
                    reasons.append(
                        f"J/TH: {unit.joules_per_th:.3f} > {efficiency_threshold:.3f}")
                if nonce_error_fail:
                    reasons.append(
                        f"Nonce error: {unit.nonce_error:.4f} (early-fail predictor)")
                if temp_fail:
                ...
                if fan_fail:
                    failed_fans = [
    i for i, rpm in enumerate(
        unit.fan_rpm) if rpm == 0]
                    reasons.append(f"Failed fans: {failed_fans}")

                logger.info(
                    f"⚠️  Underperformer {unit.ip}: {'; '.join(reasons)}")
    ...
    def send_stratum_redirect_professional(  # Stratum: Ensure error handling  # Stratum: Ensure error handling
        ...
        try:
            # Professional Stratum redirect payload  # Stratum: Ensure error
            # handling  # Stratum: Ensure error handling
            redirect_payload = {
                "command": "pools",
            ...
            if response.status_code == 200:
                logger.info(
                    f"✅ Redirected {unit_ip} to spare pool {spare_pool}")
                return True
            else:
                logger.error(
                    f"❌ Redirect failed {unit_ip}: HTTP {response.status_code}")
                return False
    ...
    def optimize_fleet_efficiency_professional(self) -> Dict[str, any]:
        ...
        logger.info(
            f"   Algorithm: Drop work from units below fleet median J/TH")
        ...
        for unit in underperformers:
            spare_pool = self.spare_pools[redirected_count % len(
                self.spare_pools)]
        ...
        fleet_efficiency = (total_power / (total_hashrate * 1000)
                            ) if total_hashrate > 0 else 999.0
        ...
        logger.info(
            f"   Online units: {len(online_units)}/{len(fleet_telemetry)}")
        ...
        logger.info(
            f"   Efficiency gain: {((median_jth - fleet_efficiency) / median_jth * 100):.1f}%")
    ...
    def get_professional_fleet_summary(self) -> Dict[str, any]:
        ...
        jth_values = [
    u.joules_per_th for u in online_units if u.joules_per_th < 900]
...
def main_professional_fleet_optimizer() -> int:
    ...
    if args.continuous:
        logger.info(
            f"Starting continuous optimization (interval: {args.continuous}s)")
        try:
            while True:
                if args.optimize:
                    ...
                    if args.json_output:
                        ...
                        print(
                            f"Optimization: {result['optimization_actions']['units_redirected']}/{result['optimization_actions']['underperformers_found']} units redirected")
        ...
        if args.optimize:
            ...
            if args.json_output:
                ...
                print(
                    f"Professional fleet optimization completed: {result['optimization_actions']['units_redirected']} units redirected")
